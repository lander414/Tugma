...
{
    include("../db/database.php");
    $query = "SELECT AVG(rating) AS averageRating FROM ratings WHERE userid = $userid";
    $result = mysqli_query($mysqli, $query);
...
function GetTopRatedUsers() {
  global $mysqli;
  $sql = "SELECT users.userid, users.username_, profilephoto.extension, AVG(ratings.rating) as avg_rating
          FROM users
